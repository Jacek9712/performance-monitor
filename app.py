...
LISTA_ZAWODNIKOW = sorted([
    ...
    "Oskar Mazurkiewicz", "Sebastian Steblecki", "Szymon Zalewski", "Tomasz Wojcinowicz", "Aleksander Wołczek", "Jakub Apolinarski", "Arkadiusz Najemski", "Oleksandr Azatskyi", "Mikołaj Baran", "Antoni Młynarczyk", "Kacper Wybieralski", "Karol Kalata", "Filip Rejczyk", "Kacper Szymanek"
])
...
if "wellness_sent" not in st.session_state: st.session_state.wellness_sent = False
if "rpe_sent" not in st.session_state: st.session_state.rpe_sent = False
if "gym_sent" not in st.session_state: st.session_state.gym_sent = False
...
def save_to_gsheets(row_data):
    try:
        ...
        if row_data['Typ_Raportu'] == "Wellness":
            ...
            st.session_state.rpe_sent = True
...
def check_today_gym_report(zawodnik):
    if st.session_state.get("gym_sent", False): return True
    try:
...
def save_gym_to_gsheets(row_data):
    try:
        ...
        # ZAMIAST CZYŚCIĆ CACHE WSZYSTKIM ZAWODNIKOM: Zmieniamy status lokalnie
        st.session_state.gym_sent = True
...
if current_player:
    ...
    if st.button("Wyloguj (Zmień zawodnika)", use_container_width=True):
        ...
        st.session_state.wellness_sent = False
        st.session_state.rpe_sent = False
        st.session_state.gym_sent = False
        st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
...
### Krok 2: Aplikacja Sztabu (`sztab_app.py`)

```python:Warta Poznań Sztab:sztab_app.py
import streamlit as st
from streamlit_gsheets import GSheetsConnection
import pandas as pd
                        ...
                        if not plany_dnia.empty:
                            for _, p in plany_dnia.iterrows():
                                ...
                                if regen and regen != 'nan':
                                    ikona = "🌿"
                                    # Pobranie czystej treści dla odnowy, żeby nie wyświetlało brzydkich tagów
                                    krotki_regen = parsuj_cwiczenie(regen)["nazwa"]
                                    krotki_regen = krotki_regen[:35] + "..." if len(krotki_regen) > 35 else krotki_regen
                    ...
                    with col_gps_main:
                        ...
                        if st.button("💾 Zapisz DZISIEJSZE dane GPS do Bazy Danych", use_container_width=True):
                            with st.spinner("Zapisywanie w bazie..."):
                                ...
                                # BEZPIECZNY ZAPIS BAZY GPS
                                try:
                                    df_historia = conn.read(worksheet="Historia_GPS", ttl=0)
                                    if df_historia is None: df_historia = pd.DataFrame()
                                except Exception as e:
                                    st.error("❌ Przeciążenie serwerów Google (Błąd 429). Wstrzymano zapis GPS, aby chronić starą bazę. Spróbuj za minutę.")
                                    st.stop()
                ...
                if st.button("🚀 Uruchom Archiwizację Historyczną", type="primary"):
                    ...
                    if ilosc_dni < 0:
                        ...
                        if historia_danych:
                            ...
                            # BEZPIECZNY ZAPIS MASOWY
                            try:
                                df_istniejaca = conn.read(worksheet="Historia_GPS", ttl=0)
                                if df_istniejaca is None: df_istniejaca = pd.DataFrame()
                            except Exception as e:
                                st.error("❌ Błąd połączenia z Google Sheets (Limit zapytań). Operacja wstrzymana, spróbuj ponownie za minutę.")
                                st.stop()
            ...
            with tab_ai_log:
                ...
                with st.form("injury_form", border=True):
                    ...
                    if st.form_submit_button("ZAPISZ URAZ W BAZIE ML"):
                        ...
                        # BEZPIECZNY ZAPIS URAZU
                        try:
                            df_urazy = conn.read(worksheet="Urazy", ttl=0)
                            if df_urazy is None: df_urazy = pd.DataFrame()
                        except Exception as e:
                            st.error("❌ Przeciążenie serwerów Google (Błąd 429). Operacja zapisu urazu wstrzymana. Spróbuj za minutę.")
                            st.stop()
